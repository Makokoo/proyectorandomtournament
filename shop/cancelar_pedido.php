...
$desactivar = "SET FOREIGN_KEY_CHECKS=0";
$activar = "SET FOREIGN_KEY_CHECKS=1";
$lineas = "DELETE FROM lineas_pedidos WHERE cod_pedido=".$_POST['cod'];
$pedido = "DELETE FROM pedidos WHERE cod_pedido=".$_POST['cod'];

$conexion->query($desactivar);
$lin = $conexion->query($lineas);
$ped = $conexion->query($pedido);
if($conexion->affected_rows==0){
    $conexion->query($activar);
    $conexion->rollback();
    echo "<label class='alert-warning'>No se ha podido borrar el pedido</label>";
    if(verpermiso($_SESSION['nick'],$conexion)>0) {
        header("Refresh:2; url=gestion_pedidos.php");
    }else{
        header("Refresh:2; url=verpedidos.php");
    }
}else {
    if ($conexion->affected_rows == 0) {
        $conexion->query($activar);
        ...
        $conexion->query($activar);
        $conexion->commit();
        echo "<label class='alert-success'>El pedido se ha borrado correctamente</label>";
        if(verpermiso($_SESSION['nick'],$conexion)>0) {
            header("Refresh:2; url=gestion_pedidos.php");
        }else{
            header("Refresh:2; url=verpedidos.php");
